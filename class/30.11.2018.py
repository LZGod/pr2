...
def json_maker():
    dict_csv = {}
    fieldnames = ['ask', 'what', 'how', 'name', 'surname']
    with open(filename, 'r+', encodind="utf-8") as csvfile:
...
def show_result():
    ...
    if request.method =='POST':
        ...
        with open(filename, 'r', encoding = 'utf-8') as csvfile:
            ...
            for row in reader:
                if row['ask']==ask:
                    if (row['what'] == what or row['how']==what
                        or row['name']==what or row['surname']==what):
                        name = row['name']+' '+row['surname']
                        dict_csv[name]=json.loads(json.dumps(row))
